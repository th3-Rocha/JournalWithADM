...
type NewsTextProps = {
  data: NewsResponseProps;
};

export default function NewsText({ data }: NewsTextProps) {
  const initialValue = data.text;
  ...
  const handleBackToNewsListPage = () => router.push(`/admin/posts`);

  const handleSubmit = async (event: FormEvent) => {
    event.preventDefault();

    const response = await updateNewsText(id, value);
        ...
        <div>
          ...
          <div className={styles.contentButton}>
            ...
            <QuestionModal
              ...
              submit
              title="Salvar post"
              body="Você realmente quer salvar o post?"
              btnLeft="Voltar"
              btnRight="Salvar"
